(ns com.lemondronor.turboshrimp.video
  ...
  (:import (java.awt Graphics)
           ...
           java.awt.FlowLayout)
  (:require [com.lemondronor.turboshrimp.decode :as decode])
...
(def ^JFrame window (JFrame. "test"))
(def ^JPanel view (JPanel. ))
...
(defn setup-viewer []
  ...
  (def g ^Graphics (.getGraphics view)))
...
(defn read-from-input [size]
  (let [bv (byte-array size)]
    (.read (.getInputStream ^Socket @vsocket) bv)
    bv))
...
(defn display-frame [video]
  (try
    (let [buff-img ^BufferedImage (decode/convert-frame video)]
      (def my-img buff-img)
      (update-image
       ;;(if @opencv
       ;;  (opencv/process-and-return-image buff-img)
         buff-img))
       ...
       (if (= "PaVE" (read-signature vheader))
         (do
           (let [vpayload (read-payload (payload-size vheader))]
             ;;(println (take 6 (seq vpayload)))
             (when out
               (write-payload vpayload out))
             (when (= (nth vpayload 4) 103)
               (reset! frame-counter -2))
             (swap! frame-counter inc)
             (when (or (= (nth vpayload 4) 103)
                       (= (mod @frame-counter 4) 0))
               ;;(println :displaying)
